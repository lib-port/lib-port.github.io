...
test("uses an absolute date for a future calendar day", () => {
  ...
  assert.equal(formatPushedAt("2026-08-04T23:00:00Z", now), "Updated on Aug 5");
});
...
test("counts calendar days across a daylight-saving transition", () => {
  ...
  assert.equal(
    formatPushedAt("2026-03-28T12:00:00+01:00", now),
    "Updated 2 days ago"
  );
});
...
test("chooses the displayed year from local dates", () => {
  ...
  assert.equal(formatPushedAt("2025-12-31T23:30:00Z", now), "Updated on Jan 1");
  assert.equal(
